...
//  export const db = new AstraDB(process.env.CASSANDRA_TOKEN, process.env.CASSANDRA_API_ENDPOINT, process.env.CASSANDRA_KEYSPACE);
...
function createCassandraClient(): Client {
  const newClient = new Client({
    cloud: {
      secureConnectBundle: path.join(process.cwd(), "secure-connect.zip"),
